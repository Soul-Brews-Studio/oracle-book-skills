...
| output | one `.pdf` **committed to the repo** (`ψ/writing/mini-books/<slug>/`) + source `.md`, then copied to `~/Downloads/` for easy opening |
...
### Step 6 — Deliver (repo first, Downloads second — Nat, 2026-07-13)
**The repo is home. `~/Downloads/` is just a convenience copy, never the only copy.**
1. `git add` the PDF (and the source `.md` files) inside the oracle repo's `ψ/writing/mini-books/<slug>/`, commit, and push. This is the one vault-write exception to "do NOT `git add` ψ/" — retrospectives/memory/handoffs stay uncommitted shared state, but a finished **book/booklet PDF is a deliverable**, not ephemeral state, and must live in git history.
2. **Then** `cp "$DIR/$SLUG.pdf" ~/Downloads/` so it's easy to double-click open.
3. Optionally `maw hey` a peer.

If the PDF ends up in `~/Downloads/` without a matching commit in the repo, the step isn't done — go back and commit it.
...
# 8. deliver — REPO FIRST, Downloads is just a convenience copy (Nat, 2026-07-13)
git -C "$(git rev-parse --show-toplevel)" add "$DIR/$SLUG.pdf" "$DIR"/*.md "$DIR/preamble.typ" 2>/dev/null
git -C "$(git rev-parse --show-toplevel)" commit -m "booklet: $TITLE" 2>/dev/null
git -C "$(git rev-parse --show-toplevel)" push 2>/dev/null
cp "$DIR/$SLUG.pdf" ~/Downloads/
echo "✅ $DIR/$SLUG.pdf → committed to repo, then → ~/Downloads/  (now Read the PNGs above with the Read tool)"
```
